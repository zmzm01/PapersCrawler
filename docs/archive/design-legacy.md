...
阶段耗时来自内存中的阶段结果。ntfy 通知不发送论文级统计、错误摘要或起止时间，详细原因统一通过本地日志查看。

`src/processors/ntfy_notifier.py` 使用 `requests.post`、`Markdown: yes`、`Content-Type: text/markdown` 和可选 `Authorization: Bearer ...`。通知正文只使用 ntfy 官方 Markdown 子集，保留运行状态、总耗时以及实际执行阶段的状态和耗时；详细错误通过本地日志查看。它对 UTF-8 正文做保守截断。任何请求异常只记录日志并返回 `False`，不得影响数据库写入或流水线退出。
...
│   ├── test_web_review.py       # 人工相关性审核 WebUI
...
│   ├── main.py                  # 兼容旧 CLI 入口 (委托 pipeline.runner)
│   ├── db/
...
│       ├── templates/           # Jinja2 模板（监控/报告/人工审核）
│       │   ├── base.html        #   布局模板
│       │   ├── dashboard.html   #   状态概览
│       │   ├── papers.html      #   论文浏览
│       │   ├── report.html      #   报告阅览与下载
│       │   ├── relevance_review.html        # 人工审核队列
│       │   └── relevance_review_detail.html # 人工审核详情
│       └── static/
│           ├── css/style.css
│           └── js/app.js
├── tools/                       # 辅助工具
│   ├── reset_pipeline.py        # 重置流水线状态（6 子命令 + --publisher 过滤）
│   ├── run_pipeline.py           # 推荐统一 CLI/cron 入口
...
主表 `papers` 每篇论文一行，按阶段添加字段；独立审计表记录人工相关性审核。每个阶段有三态状态列（status + error + date）：
...
**相关性人工审核** — 独立表 `relevance_reviews`
- 记录 `doi`, `decision`（A/B/C/D/uncertain）、`notes`, `reviewer`, `created_date`
- 同时快照提交时的 `llm_relevance_category` 与 `llm_relevance_confidence`，保留模型判断上下文
- 表为追加式历史；当前审核结果取同一 DOI 最大 `id` 的记录，不覆盖 `papers` 中的 LLM 字段
...
自动流水线生成的日报（Phase G）和历史用户报告写入不同目录，避免邮件推送误发非自动报告。
当前 WebUI 只提供报告阅览与下载，不提供勾选生成新报告的功能；新的预览报告应通过
`tools/preview_report.py` 生成。

| 维度 | 自动报告 (Phase G) | 历史用户报告（仅存档） |
|------|-------------------|-------------------|
...
> **Web UI 是监控、报告阅览与相关性人工审核工作台。** Dashboard、Papers、Report 保持只读；人工审核页仅写入追加式审核记录，不修改流水线状态、配置或 LLM 原始结果。所有配置修改走配置文件，所有运行控制走 `tools/run_pipeline.py`。Home / Pipeline / Logs 页面已在 2026-07-26 瘦身中删除。
...
| Relevance Review | `GET /relevance-review` / `GET /relevance-review/{doi}` | 正文终审结果的人工审核队列与详情；支持按审核状态、类别、置信度、初筛/终审分歧和标题/DOI 筛选；提交结果写入 `relevance_reviews` 审计表 |

## WebUI 安全

WebUI 的 Dashboard、Papers、Report 为只读；人工审核页保留一个受限 POST 端点，仅接受固定决策值和长度受限文本，并实施以下安全加固：
...
| R3 | 审核输入与全文路径校验 | `POST /api/relevance-reviews` 校验 DOI、A/B/C/D/uncertain、备注/审核人长度；详情页只按数据库中的 `mineru_output_dir` 读取 `data/` 内 `full.md`，不接受任意文件路径 |
...
阶段开关统一配置在 `configs/settings.yaml` 的 `skip_phases` 节点，
`src/config.py` 负责读取并写入运行时配置对象 `CFG`：
...
推荐 CLI 入口是 `tools/run_pipeline.py`。`--daily` 和 `--weekly` 尊重这些开关；
`--all` 使用 `force=True`，忽略开关执行全部阶段。WebUI 不启动或控制流水线，也不会单独
覆写阶段开关；Dashboard/Papers/Report 读取数据库和报告文件，Relevance Review 只向
`relevance_reviews` 追加审核记录。

配套 `pipeline.max_papers_per_phase` 控制每阶段处理上限（0 = 不限制）。

## journal_overrides.json

`data/journal_overrides.json` 是可选的期刊启用/禁用覆写文件，当前没有 WebUI 页面编辑它：
...
每日/每周调度（`force=False`）不读取此文件，只读 `configs/publishers.yaml`；
`tools/run_pipeline.py --all`（`force=True`）在 Phase A 读取它。
...
**Config 开关**：`configs/settings.yaml` 中 `pipeline.skip_nature_news: true`（默认开启）。关闭后 Nature 新闻类文章
...
`data/journal_overrides.json` 是可选的期刊启停覆写文件。当前 WebUI 已删除 Data Sources
页面，因此该文件需要手工维护；普通每日/每周调度不读取它。
...
- 全流程入口（`tools/run_pipeline.py --all`，`force=True`）→ `use_overrides=True` → 加载 overrides 叠加到 publishers.yaml
...
```bash
LOG_LEVEL=INFO python tools/run_pipeline.py --daily
```
...
历史 WebUI 配置页的多个 save 端点曾统一使用 `_atomic_write()`（先写 .tmp 再 `os.replace()`），
防止并发读写导致配置损坏；配置页现已删除，当前配置应直接编辑 YAML 文件。

## Logger 架构统一

`logging.basicConfig` 从 `base.py` 移入 CLI 入口 `tools/run_pipeline.py`、兼容入口 `main.py`
和 `web/app.py`，各 phase 模块使用 `logging.getLogger(__name__)` 获取各自 logger，日志中可区分来源。
支持 `LOG_LEVEL` 环境变量控制级别。

## 配置热加载

`config.py` 提供 `reload_config()` 函数，供需要长驻运行时重新读取 YAML 配置；当前 WebUI 不提供配置编辑页面，
修改配置后应重启 CLI/WebUI 进程，或由调用方显式触发 reload。运行时配置统一通过 `CFG` 访问。
...
`phase_a_rss()` / `phase_a_crossref()` 仍接收内部参数 `use_overrides`；runner 将 `force`
传入该参数，使 `--all` 能读取可选的 `journal_overrides.json`。配套的 `skip_overrides.json`
已删除。`run_pipeline(run_all=True)` 等效于 `run_phases(force=True)`，`force` 保留兼容。

# WebUI 历史演进与当前边界

2026-07-26 起 WebUI 删除了 SSE 日志流、子进程运行控制、Config 和 Data Sources 页面，
并收敛为 Dashboard、Papers、Report 三个只读页面。2026-08-20 新增 Relevance Review 页面：
该页面只追加写入 `relevance_reviews` 审核记录，不修改流水线状态、配置或 LLM 原始结果。
运行、配置修改和邮件模板选择仍应通过 CLI、YAML 配置文件及
