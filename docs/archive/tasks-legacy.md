...
## 2026-08-20：核心文档一致性修复

- **问题**：`docs/usage.md` 残留已删除的 `/config`、`/subscriptions`、WebUI 生成报告流程；配置示例遗漏 E3/全文下载/ntfy，并将 `generate_explained_html` 放在错误层级。`docs/design.md` 还保留旧的 WebUI 只读、Data Sources 页面和旧 CLI 入口描述。
- **修复**：统一推荐入口为 `tools/run_pipeline.py`，明确 WebUI 当前提供 Dashboard、Papers、Report 和 Relevance Review；补齐 `relevance_reviews` Schema、人工审核及历史用户报告说明；将 `journal_overrides.json` 改为可选手工覆写，并说明仅 `--all` 读取。
- **同步**：保留 `docs/reviews/` 历史审查归档链接，修正 SMTP 故障排查、数据库锁说明和 `skip_nature_news` 配置位置；README 无需结构性改动。

## 2026-08-20：相关性人工审核 WebUI

- **背景**：近期 LLM 相关性判断需要人工抽查，原 WebUI 只能查看论文列表和报告，无法并排查看摘要、正文证据与两阶段 LLM 结果，也没有审核留痕。
- **设计**：新增 `/relevance-review` 队列和 `/relevance-review/{doi}` 详情页，默认面向正文终审完成的论文；未审核 B/中置信度、初筛/终审分歧、A/中置信度依次优先。
- **审计**：新增 SQLite `relevance_reviews` 追加式表，保存 A/B/C/D/uncertain、备注、审核人及提交时的 LLM 类别/置信度快照；人工结论不会覆盖 `papers` 原始 LLM 字段。
- **安全**：新增受限 `POST /api/relevance-reviews`，校验决策值和文本长度；MinerU 正文只允许从 `data/` 下数据库记录的 `full.md` 读取，并复用路径边界校验。
- **验证**：新增数据库队列优先级、审核历史和快照测试；`pytest -q tests/test_db.py` 通过 36 项。当前系统环境缺少 FastAPI/Pydantic，Web 路由测试需在安装 `requirements.txt` 后执行。

## 2026-08-19：Publisher 失败排查与 ntfy 通知精简

- **日志结论**：近期失败只有 IOP 的 `10.1088/1361-6587/ae9985` 和 `10.1088/1361-6587/ae9ac7`。两篇均返回 Radware Bot Manager Captcha；`curl_cffi` HTTP fallback 虽返回 200，但仍是验证码页，标题、DOI 和摘要均为空，最终以 `PageParseError` 失败。数据库中对应错误为 `Title, DOI and Abstract all empty (bot block)`。
- **通知调整**：ntfy 保留运行状态、总耗时和实际执行阶段的状态/耗时；移除起止时间、E/E3/F 统计和错误摘要，详细原因改为查看本地日志。
- **验证**：新增/更新离线测试覆盖精简正文，使用项目 Python 环境执行测试。
...
- 修复：模板在各结构边界显式输出空行；新增回归测试验证引用块不粘连、一级标题独占一行。
- 同时修复解释页日期解析：Phase G 输出 `report_YYYYMMDD_explained.html`，旧解析器只接受带连字符格式而产生无害 warning；现兼容两种格式并统一输出 ISO 日期。
