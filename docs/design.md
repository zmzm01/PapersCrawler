...
审核队列的详情、分页、随机审核与返回导航必须安全编码 DOI，并携带当前审核模式及筛选上下文，避免进入详情后静默重置用户已经设置的筛选条件。
队列行同时提供整行键盘/鼠标入口和显式审核按钮；窄屏将表格转为带字段标签的卡片。详情保存期间禁用重复提交，并提供仅保存与保存后继续审核的键盘快捷键。
...
`data/reports/user/` 是组内特别报告的隔离目录，部署脚本没有将其导出的选项。历史重建默认直接同步到仓库内站点的 JSON 与 Markdown 下载目录，不再写入旧的同级 `MySite`；若某个日期窗口没有合格论文，不会生成空 Markdown 或 sidecar，若重建已有的空报告，会一并清理其解释页，避免公开站点出现零条目页面。
...
站点吸顶行为只属于布局层的 `.site-header`。内容页面可以使用语义化 `<header>`，但不得继承导航栏的 `position: sticky`、背景模糊或层级样式。

### 公开方法页与文本拼接契约

- `/methodology/` 公开 Phase F 当前的总结系统 Prompt，并单独说明运行时将“标题 + 全文”作为用户输入；不得把系统 Prompt 误称为包含论文正文的完整请求。
- 方法页必须容忍缺失或无效的生成时间、空指纹和空 Prompt，长 Prompt 在窄屏下不得撑破页面。
- 结构化字段由渲染器连接时，连接词负责提供边界标点。若“局限”文本已以句号、分号、逗号、冒号或问叹号结尾，应先去掉末尾边界标点，再连接 `；影响：`，避免 `。；`、`。：` 等重复标点。
