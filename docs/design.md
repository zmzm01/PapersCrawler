...
`MinerUParser._request_with_retry()` 提供统一的 3 次重试封装，覆盖 create_batch / upload / poll / download 四个步骤。
...
:  以及 `cf-ray` + 短 HTML（<2000 字符）组合、`turnstile` + `challenge` 组合
