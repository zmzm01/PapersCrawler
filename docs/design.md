...
Phase H 检测逻辑：
1. 检查 `auto/report_YYYYMMDD.md` 是否存在
2. 存在 → 作为附件发送，正文使用 HTML 模板渲染
3. 不存在 → 发送无更新通知（同样使用 HTML 模板）

**HTML 邮件模板**：`templates/email/default.html`
- 使用 `str.format()` 渲染，不引入新依赖
- 模板变量：`{report_title}`（邮件标题）、`{paper_count}`（论文数）、`{has_papers}`（是否有论文）
- 报告作为附件，正文无论文列表
- 模板名可在 `configs/settings.yaml` 的 `email.template` 配置，WebUI Config 页面可覆盖
