...
| 模块 | 变更 | 日期 |
|------|------|------|
| **keywords.yaml 本地重构** | `configs/keywords.yaml`（gitignore 排除，本地运行时配置）实施 7 项重构：(1) 删 `sub_domains_embedding` 整段 47 行（Phase D 已删，配置无读取方，src/ grep 0 引用）；(2) 删 `priority_hint` 字段（4 个子域）+ 删 6 行模板注释（仅渲染为 LLM 软文本，无规则分支，src/ grep 0 引用，本批 commit 同步清理 `src/config.py:460-463` 渲染块）；(3) `advanced_technology` 4→3 拆：`beam_physics`（束流传输中尾场耦合/空间电荷/发射度，description 显式"不包含纯电子 LWFA"配合 context_gates 双重保险）/ `beam_optics_technology`（APL/毛细管/可变形镜/束流线等 6 项）/ `accelerator_intelligence`（ML/数字孪生/自动化 3 项）；(4) `plasma_physics` 拆 5 子域：`laser_plasma_interaction`（自聚焦 1 条）/ `plasma_channel`（通道形成+毛细管放电源 2 条）/ `plasma_target`（气体靶 1 条，后续可补 foil/cryogenic 等）/ `plasma_diagnostics`（4 条）/ `plasma_simulation`（FLASH + radiation hydro 2 条）；(5) `context_gates` 加 `diagnostics` 消歧规则（天体物理/聚变/医疗/半导体等诊断排除）；(6) `acceleration.description` 补 CSA + near-critical 描述；(7) `beam_applications.description` 放宽应用范围 + 新增 1 条 topic `Warm dense matter and high energy density physics (HEDP) generation`。历史数据策略：174 篇 `papers.db.llm_relevance_subfields` 中 `plasma_physics` 旧标签**不迁移不重跑**（用户决策），新 scope 校验会过滤旧 key。测试：所有 test 用硬编码 fixture 不读 DB，160 passed 仍生效。 | 07-25 |
