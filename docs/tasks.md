...
## 2026-09-15：报告站长文阅读、公式与项目环境优化

- 保留现有 C 类卡片展示，新增报告内全文元数据搜索与 A/B/C 组合过滤、匹配计数、移动端悬浮目录/返回顶部、复制 DOI/链接、篇序、下一篇操作和展开偏好记忆。
- 归档与详情使用日期化周报标题，归档卡片直接展示 A/B/C 数量和首篇核心推荐；重复折叠控件加入论文上下文，改善屏幕阅读器辨识。
- 公式改为 Astro 构建期 KaTeX 静态 HTML，移除线上 MathJax CDN；APS 摘要改为保留后代文本，历史公式空洞在报告重建时优先从 MinerU Abstract 恢复。
- 在仓库下创建 Python 3.12 `.venv` 并安装 `requirements.txt`；`.venv/` 加入忽略规则，daily/weekly 包装脚本默认使用项目环境，仍允许 `PAPERSCRAWLER_PYTHON` 覆盖。
- Review 后修正筛选场景下“下一篇”误跳隐藏论文的问题，并让末篇按钮随当前结果动态隐藏；安装文档补充 `uv` 前置说明及标准库 `venv` 备用流程。
- Python 完整回归测试 496 项通过；Astro 类型检查和生产构建通过。

## 2026-09-15：修复方法页标题区错误吸顶

- 根因是全局 `header` 选择器同时命中站点导航和 methodology 页的语义化标题区，导致“报告如何生成”随页面滚动悬置。
- 站点顶栏增加专用 `.site-header`，吸顶、背景和打印隐藏规则只作用于该类；页面内容 `<header>` 恢复普通文档流。

## 2026-09-14：方法页与报告标点复查

- 发现方法页把 Phase F 系统 Prompt 笼统称为完整 Prompt，未交代每篇论文实际还会以“标题 + 全文”作为用户输入；同时日期与指纹展示对异常导出值不够稳健。
- 发现结构化局限文本通常自带句号，而 Markdown/HTML 历史模板及静态站点又直接追加“影响”，造成 `。；影响` 或 `。：影响`。
- 三条展示路径现统一规范字段边界，不修改 LLM 保存的原始结构化内容；方法页准确拆分系统指令与输入封装，并对窄屏和缺失元数据安全降级。
- 历史重建时发现工具仍默认导出到已退出当前链路的同级 `MySite`；默认值已改为仓库内 `report-site/src/data/reports`，并同步维护 `report-site/public/downloads`。
- 14 期历史报告与下载文件已重建，重复连接标点扫描结果为 0；Python 493 项测试和 Astro 17 文件检查通过。

## 2026-09-14：公开报告方法页与高收益阅读优化

- 新增公开“报告方法”页，构建时读取当前实际生效的 Phase F 总结 Prompt，并展示处理流程、字段含义、生成时间和 Prompt 指纹；仅概述相关性筛选，不公开研究范围词表和完整相关性 Prompt。
- 方法页明确当前 Prompt 不等同于所有历史报告的原始快照；本地逐期 explained HTML 保持兼容，Cloudflare Pages 不导出这些旧解释页。
- 保留现有视觉主题，优化分类扫读、目录定位、正文行宽、展开控制、移动端、键盘焦点、减少动效和公式按需加载。
- 方法数据缺失时可安全降级构建；完整部署验证当前 Prompt 可用并生成 17 个静态页面。Python 490 项测试及 Astro 检查、生产构建全部通过。

## 2026-09-14：全量历史报告重建与 Markdown 下载

- 历史报告不再信任旧 Markdown/sidecar 内容，只保留已有报告日期作为窗口边界；默认跨 `legacy/auto` 按当前数据库状态重新筛选和生成。
- 2026-06-08 累计首期仅收录 A/B，避免加入 175 篇累计 C；其余窗口全部加入 C 类邻近观察。
- Cloudflare Pages 报告详情页增加公开 Markdown 下载，导出时同步清理陈旧下载文件；特别报告和解释页继续隔离。
- 全量重建得到 14 期、合计 172 篇次；首期 15 篇 A/B，其余各期 3–20 篇 A/B/C。Python 488 项测试、Astro 检查和生产构建通过。

## 2026-09-12：优化人工审核 WebUI 操作效率

- 审核队列增加整行鼠标/键盘进入、显式“审核”按钮和一键清空筛选，降低长标题和密集表格下的点击成本。
- 窄屏队列改用带字段标签的卡片布局，中等宽度隐藏低优先级“依据”列；筛选区和分页同步适配小屏。
- 详情页保存时锁定按钮，避免重复提交；增加 `Ctrl/Cmd+S` 仅保存和 `Ctrl/Cmd+Enter` 保存并随机下一篇快捷键。

## 2026-09-12：修复人工审核导航与筛选状态回归

- 修复审核队列直接拼接 DOI 导致含 `?` / `#` 的条目链接被浏览器截断的问题，详情 URL 统一安全编码。
- 队列 → 详情 → 返回、模式切换、分页、随机审核及“保存并随机下一篇”现在保留状态、分类、置信度、不一致、搜索、排序和每页数量等筛选上下文。
- 新增特殊 DOI 与筛选保留回归测试；WebUI 定向测试 25 项、完整离线测试 480 项通过。
...
## 2026-09-14：报告隐藏无内容字段

- Markdown/HTML 结构化章节统一过滤空字符串及完整值为 `未提供`、`暂无`、`无` 的占位字段；章节没有有效子项时不再显示标题，也不再补写“正文未提供……”等展示层占位句。
- 公开 JSON sidecar 递归删除精确占位字段和空容器，Cloudflare Pages 同时在读取层过滤旧报告中的占位值，因此新旧报告都不会再堆叠“未提供”。含“部分参数未提供”等实质句子保留。
- 缺失日期不再强制显示“未知”；数据库中的规范化 Summary 仍保留固定 schema，不改变 Phase F 质量门禁和历史数据。
- 为 Markdown/KaTeX 渲染脚本补齐 JSDoc 类型、异常类型收窄和 `marked` 同步返回约束，清除 Astro check 原有的 10 个 TypeScript 错误。

## 2026-09-12：C 类作为轻量邻近观察进入报告

- 周报继续把 A/B 作为正式推荐并要求 Phase F 完整总结；有效 C 不进入总结队列，而是从 `pipeline.report_adjacent_observation_since` 起以终审理由、原文摘要和链接组成轻量条目，避免额外 LLM 总结成本。
- 报告头部新增 C 类数量与独立“邻近观察”清单，并明确说明该策略用于缓解严格 A/B 口径导致的条目过少，同时为 prompt 设计边界和表达局限可能造成的遗漏提供人工观察缓冲；C 不等同于正式推荐。
- 自动报告、用户选定报告、预览、Markdown/HTML 和公开 JSON 统一支持 A → B → C；启用日期默认 `2026-09-07`，避免历史 C 首次上线时集中回灌。
...
- 清理代码、systemd 模板和文档中的机器专属绝对路径；`.env`、SQLite、报告输出和运行时数据继续由 Git 忽略。
- 公开技术配置与研究关键词保留；个人邮箱、凭据和本机痕迹不进入公共仓库历史。
