...
审核队列会在打开详情、返回队列、翻页及随机审核时保留当前模式和筛选条件。
标题链接、整行点击或聚焦行后按 Enter 均可进入审核详情，操作列也保留显式“审核”入口；“清空筛选”只重置当前模式的筛选条件。移动端队列显示为带字段名的卡片。详情页可用 `Ctrl+S`（macOS 为 `Cmd+S`）仅保存，或用 `Ctrl+Enter` / `Cmd+Enter` 保存并随机进入下一篇；保存期间按钮会暂时禁用以避免重复提交。
