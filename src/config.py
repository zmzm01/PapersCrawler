...
# 非研究论文标题前缀检测 — pre-fetch（浏览器启动前根据 DB 标题直接删除）
PREFETCH_NON_RESEARCH = True
# 非研究论文标题前缀检测 — post-fetch（抓取解析完成后再次兜底检测）
POSTFETCH_NON_RESEARCH = True
# 标题前缀列表（大小写不敏感，前缀匹配 startswith）
...
PREFETCH_NON_RESEARCH = _pp.get("prefetch_non_research", PREFETCH_NON_RESEARCH)
POSTFETCH_NON_RESEARCH = _pp.get("postfetch_non_research", POSTFETCH_NON_RESEARCH)
NON_RESEARCH_KEYWORDS = _pp.get("non_research_keywords", NON_RESEARCH_KEYWORDS)
